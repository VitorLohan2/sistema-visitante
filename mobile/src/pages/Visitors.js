...
const loadVisitors = async () => {
  try {
    ...
    Alert.alert('Erro', err.response?.data?.message || 'Erro ao carregar visitantes');
  } finally {
    setLoading(false);
    setRefreshing(false);
  }
  ...
  const renderItem = ({ item, index }) => (
    <View style={styles.item}>
      <Text style={styles.itemText}>{index + 1}. {item.name || 'Visitante'}</Text>
      <Text style={styles.itemSubText}>CPF: {item.cpf || 'Não informado'}</Text>
      <Text style={styles.itemSubText}>Empresa: {item.company || item.empresa || 'Não informado'}</Text>
      <Text style={styles.itemSubText}>Setor: {item.sector || item.setor || 'Não informado'}</Text>
      ...
      {!item.exit_date && (
        <TouchableOpacity
          style={styles.button}
          onPress={() => handleEndVisit(item.id)}
        >
