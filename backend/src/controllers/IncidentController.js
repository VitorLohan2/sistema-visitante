...
module.exports = {
  ...
  async create(request, response) {
    ...
    try {
      ...
      console.log("✅ Visitante cadastrado no banco:", incident.id);

      io.to("global").emit("visitante:create", {
        id: incident.id,
        ong_id,
      });
  ...
  async update(req, res) {
    ...
    try {
      ...
      const currentIncident = await connection("incidents")
        .where("id", id)
        .select("imagem1", "imagem2", "imagem3", "ong_id")
        .first();
      ...
      // ✅ USAR O ONG_ID DO BANCO (MAIS CONFIÁVEL)
      const targetOngId = currentIncident.ong_id;

      console.log(`📡 Emitindo visitante:update para sala: ong:${targetOngId}`);
      ...
      console.log("✅ Evento visitante:update emitido com sucesso!");
  ...
  async blockIncident(request, response) {
    ...
    console.log("🚫 Bloqueio solicitado:", { id, bloqueado, ong_id });
    ...
    try {
      ...
      // ✅ EMITIR MÚLTIPLOS EVENTOS PARA GARANTIR
      const eventData = {
        id,
        bloqueado: !!bloqueado,
        timestamp: new Date(),
      ...
      // Evento geral de bloqueio
      io.to("global").emit("visitante:block", eventData);

      // Evento específico de atualização
      io.to("global").emit("visitante:update", eventData);

      // Evento específico de bloqueio (para compatibilidade)
      if (bloqueado) {
      ...
      console.log(`✅ Eventos emitidos para sala GLOBAL:`, [
        "visitante:block",
        "visitante:update",
        bloqueado ? "bloqueio:created" : "bloqueio:updated",
      ]);

      return response.status(200).json({
        message: bloqueado
          ? "Visitante bloqueado com sucesso"
          : "Visitante desbloqueado com sucesso",
        id,
        bloqueado: !!bloqueado,
      });
    } catch (error) {
  ...
  async delete(request, response) {
    ...
    try {
      ...
      if (!incident) {
        return response
          .status(404)
          .json({ error: "Incidente não encontrado." });
      }
